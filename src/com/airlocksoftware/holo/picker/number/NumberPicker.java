 ...
 */
package com.airlocksoftware.holo.picker.number;
...
public class NumberPicker extends LinearLayout implements OnClickListener, OnFocusChangeListener, OnLongClickListener {
	...
	private final TextView mTextAbove;
	private final TextView mTextBelow;
	...
	public NumberPicker(Context context, AttributeSet attrs, int defStyle) {
		...
		mTextAbove = (TextView) findViewById(R.id.above_text);
		mTextBelow = (TextView) findViewById(R.id.below_text);
	...
	protected void changeCurrent(int current) {
		...
		current = getWrappedNumber(current);
		mPrevious = mCurrent;
		mCurrent = current;
	...
	public int getWrappedNumber(int toWrap) {
		...
		} else if (toWrap < mStart) {
			toWrap = mWrap ? mEnd : mStart;
		}
		return toWrap;
	}
	...
	protected void updateView() {
		...
		if (mDisplayedValues == null) {
			mTextAbove.setText(formatNumber(getWrappedNumber(mCurrent + 1)));
			mText.setText(formatNumber(mCurrent));
			mTextBelow.setText(formatNumber(getWrappedNumber(mCurrent - 1)));
		} else {
			mTextAbove.setText(mDisplayedValues[getWrappedNumber(mCurrent + 1) - mStart]);
