...
public class MergeAdapter extends BaseAdapter implements SectionIndexer {
  ...
  public int getViewTypeCount() {
    ...
    return(Math.max(total, 1));   // needed for setCategoryTypeGroupAdapter() before content add'
  }
  ...
  /**
    * Are all items in this CategoryTypeGroupAdapter enabled? If yes it
  ...
  // MODIFICATIONS
  /** Moves the object within the Adapters, but keeps it inside it's respective adapter **/
  public void move(int from, int to) {
	  throw new UnsupportedOperationException("Needs to be modified to work correctly");
//		Object item = getItem(from);
//		for (T piece : pieces) {
//			int size = piece.getCount();
//
//			if (from < size && to >= size) {
//				// tried to move it to the array below, move it to the end of the current array
//				piece.move(item, piece.getCount());
//				break;
//			}
//
//			if (to < size && from < size) {
//				// either moved it within the array,
//				// or tried to move it to the array above, in which case the array will move it to the front
//				piece.move(item, to);
//				break;
//			}
//
